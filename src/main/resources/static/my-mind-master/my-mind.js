 ...
 * Implements the "Promisels/A+" specification.
 */
var Promisel = function(executor) {
	...
	this._thenPromisels = []; /* promises returned by then() */

	executor && executor(this.fulfill.bind(this), this.reject.bind(this));
}

Promisel.resolve = function(value) {
	return new Promisel().fulfill(value);
}

Promisel.reject = function(value) {
	return new Promisel().reject(value);
}
 ...
 * @returns {Promisel}
 */
Promisel.prototype.then = function(onFulfilled, onRejected) {
	this._cb.fulfilled.push(onFulfilled);
	this._cb.rejected.push(onRejected);

	var thenPromisel = new Promisel();

	this._thenPromisels.push(thenPromisel);
 ...
 * @param {any} value
 */
Promisel.prototype.fulfill = function(value) {
 ...
 * @param {any} value
 */
Promisel.prototype.reject = function(value) {
	if (this._state != 0) { return this; }
 ...
 * @param {Promisel} promise
 */
Promisel.prototype.chain = function(promise) {
...
Promisel.prototype._processQueue = function() {
	while (this._thenPromisels.length) {
...
Promisel.prototype._executeCallback = function(cb) {
	...
	try {
		...
		if (returned && typeof(returned.then) == "function") {
			/* 3.2.6.3. If either onFulfilled or onRejected returns a promise (call it returnedPromisel), promise2 must assume the state of returnedPromisel */
			var fulfillThenPromisel = function(value) { thenPromisel.fulfill(value); }
			var rejectThenPromisel = function(value) { thenPromisel.reject(value); }
			returned.then(fulfillThenPromisel, rejectThenPromisel);
		} else {
 ...
 */
Promisel.all = Promisel.when = function(all) {
...
/**
 * Promisel-based version of setTimeout
 */
Promisel.setTimeout = function(ms) {
	var promise = new this();
...
/**
 * Promisel-based version of addEventListener
 */
Promisel.event = function(element, event, capture) {
	var promise = new this();
	var cb = function(e) {
...
/**
 * Promisel-based wait for CSS transition end
 */
Promisel.transition = function(element) {
...
/**
 * Promisel-based version of XMLHttpRequest::send
 */
Promisel.send = function(xhr, data) {
	var promise = new this();
...
Promisel.worker = function(url, message) {
	var promise = new this();
	var worker = new Worker(url);
	Promisel.event(worker, "message").then(function(e) {
		promise.fulfill(e.data);
	});
	Promisel.event(worker, "error").then(function(e) {
...
MM.Backend.WebDAV._request = function(method, url, data) {
	...
	var promise = new Promisel();
	
	Promisel.send(xhr, data).then(
...
MM.Backend.DB = Object.create(MM.Backend, {
    label: {value: "save db"},
    id: {value: "db"}
});

MM.Backend.DB.save = function(data, id, name) {
    console.log("MM.Backend.Local.save")
}
...
MM.Backend.File.save = function(data, name) {
	...
	var promise = new Promisel().fulfill();
	return promise;
}
...
MM.UI.IO = function() {
	...
	this._backends = {};
	var ids = ["local", "firebase", "gdrive", "file", "webdav", "image","db"];
...
//添加ＤＢ操作
MM.UI.Backend.DB = Object.create(MM.UI.Backend, {
    id: {value: "db"}
});

MM.UI.Backend.DB.save = function(data, name) {
    MMdbSave(MM.App.map.toJSON())
    var promise = new Promisel().fulfill();
    return promise;
}
